...
router.get('/:otherUserId', authenticateToken, async (req, res, next) => {
  try {
    ...
    const query = {
      $or: [{ senderId: req.user.id, receiverId: otherUserId }, { senderId: otherUserId, receiverId: req.user.id }]
    };
...
router.post('/send', authenticateToken, async (req, res, next) => {
  try {
    ...
    if (!text?.trim() || !receiverId) return res.status(400).json({ success: false, message: "Invalid payload" });
    ...
    if (isOnline && io) io.to(receiverId.toString()).emit("new-message", newMessage);

    // Async notification trigger (doesn't block response)
    // (Requires you to move your push/email logic into a helper function)
