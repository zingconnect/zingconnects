...
const handleStartCall = async () => {
  ...
  try {
    ...
    // 6. DELAYED MEDIA ATTACHMENT (The "Mismatch" Fix)
    // Move status to ringing first to show the user the call is out
    setCallStatus('ringing'); 

    // Wait 500ms before setting the token. 
    // This ensures the LiveKit room is fully indexed on the server-side cluster.
    setTimeout(() => {
      if (data.lkToken) {
